...
use structsy::{
    derive::queries,
    internal::{
        EmbeddedDescription, FieldDescription, SimpleValueTypeBuilder, StructDescription,
        ValueTypeBuilder,
    },
...
impl PersistentEmbedded for DateTime {
        ...
        Self: Sized,
    {
        ...
        read.read_exact(&mut bytes)
            .map_err(|e| structsy::StructsyError::TypeError("EOF".to_string()))?;
...
impl EmbeddedDescription for DateTime {
    fn get_description() -> structsy::internal::Description {
        structsy::internal::Description::Struct(StructDescription::new(
            "DateTime",
            &[FieldDescription::new::<u16>(
                0,
                "timestamp",
                Some(structsy::ValueMode::Cluster),
            )],
        ))
    }
}
...
mod tests {
    ...
    fn serde_events() {
        let events = [
            RingEvent {
                mac: "00:00:00:00:00:00".to_string(),
                when: DateTime(OffsetDateTime::new_utc(
