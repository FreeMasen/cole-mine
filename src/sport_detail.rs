...
impl TryFrom<&[u8]> for SportDetail {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        if value.len() < 12 {
            return Err(format!("SportDetail must be at least 12 bytes found {}", value.len()))
        }
        let mut byte_iter = value.iter().copied();
        let bcd_to_decimal = |b: u8| {
            (((b >> 4) & 15) * 10) + (b & 15)
        };
        Ok(Self::builder()
            ...
            .calories(byte_iter.next().unwrap_or_default() as u16 | (byte_iter.next().unwrap_or_default() as u16) << 8)
            .steps(byte_iter.next().unwrap_or_default() as u16 | (byte_iter.next().unwrap_or_default() as u16) << 8)
            .distance(byte_iter.next().unwrap_or_default() as u16 | (byte_iter.next().unwrap_or_default() as u16) << 8)
            .build())
    }
}
...
impl SportDetailState {
    pub fn new(packet: [u8;16]) -> Result<Self> {
        ...
        if packet[1] == 255 {
            return Ok(Self::Complete { packets: Vec::new() });
        }
        if packet[1] == 240 {
            return Ok(Self::Initial { new_cal_proto: true })
        }
        Ok(Self::Recieving { new_cal_proto: false, packets: vec![SportDetail::try_from(&packet[1..])?] })
    }
    ...
    pub fn step(&mut self, packet: [u8;16]) -> Result {
        ...
        match self {
            Self::Initial { new_cal_proto } => {
                ...
                *self = Self::Recieving { new_cal_proto: *new_cal_proto, packets: vec![packet] };
            },
            Self::Recieving { packets, new_cal_proto } => {
